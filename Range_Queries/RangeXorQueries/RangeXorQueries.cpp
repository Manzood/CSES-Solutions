...
struct segtree {  // ZERO BASED INDEXING
    ...
    long long getxor(int left, int right) {
        return getxor(left, right, 0, 0, size);
    }
    long long getxor(int left, int right, int x, int lx, int rx) {
        if (lx >= right || left >= rx) return 0;
        if (lx >= left && rx <= right) return x_or[x];
        int mid = (lx + rx) / 2;
        long long x1 = getxor(left, right, 2 * x + 1, lx, mid);
        long long x2 = getxor(left, right, 2 * x + 2, mid, rx);
        return x1 ^ x2;
    }
